...
class SSDOCCHeadV1(BaseModule):
    def __init__(self,
        ...
        if boundary_cls is not None:
            self.boundary_cls = build_loss(boundary_cls)
    ...
    def forward(self, mlvl_feats, img_metas, gt_bboxes_3d=None, gt_labels_3d=None):
        ...
        cls_scores, refine_pts, boundary_scores = self.transformer(
            init_points,
            query_feat,
            mlvl_feats,
            img_metas,
        ...
        return dict(init_points=init_points,
                    ...
                    all_boundary_scores=boundary_scores)
    ...
    def _get_target_single(self, refine_pts, gt_points, gt_masks, gt_labels):
        ...
        # cls assignment
        refine_pts_labels = gt_labels[pred_paired_idx].long()
    ...
    def loss_single(self,
        ...
        if self.transformer.with_boundary_head and boundary_scores is not None:
            boundary_scores_list = [boundary_scores[i].reshape(-1, 2) for i in range(num_imgs)]
            (labels_list_b, gt_paired_idx_list_b, pred_paired_idx_list_b, cls_weights_b,
            gt_pts_weights_b) = multi_apply(
                self._get_target_single, refine_pts_list, gt_points_list,
                gt_masks_list, gt_boundary_labels_list)
        
            boundary_scores = torch.cat(boundary_scores_list)
            boundary_labels = torch.cat(labels_list_b)
        
            loss_boundary = self.boundary_cls(boundary_scores,
                                            boundary_labels,
                                            avg_factor=boundary_scores.shape[0])
        else:
            loss_boundary = pred_pts.new_tensor(0.0)
        ...
        return loss_cls, loss_boundary, loss_pts
    
    @force_fp32(apply_to=('preds_dicts'))
    def loss(self, voxel_semantics, mask_camera, boundary_labels, preds_dicts):
        ...
        all_boundary_scores = preds_dicts['all_boundary_scores']
        
        num_dec_layers = len(all_cls_scores)
        gt_points_list, gt_masks_list, gt_labels_list, gt_boundary_labels_list = \
            self.get_sparse_voxels(voxel_semantics, mask_camera, boundary_labels)
        all_gt_points_list = [gt_points_list for _ in range(num_dec_layers)]
        all_gt_masks_list = [gt_masks_list for _ in range(num_dec_layers)]
        all_gt_labels_list = [gt_labels_list for _ in range(num_dec_layers)]
        all_boundary_labels_list = [gt_boundary_labels_list for _ in range(num_dec_layers)]

        losses_cls, losses_boundary, losses_pts = multi_apply(
            self.loss_single, all_cls_scores, all_refine_pts, all_boundary_scores, 
            all_gt_points_list, all_gt_masks_list, all_gt_labels_list, all_boundary_labels_list)
        ...
        if init_points is not None:
            ...
            _, _, init_loss_pts = self.loss_single(
                pseudo_scores, init_points, None, gt_points_list, 
                gt_masks_list, gt_labels_list, None)
        ...
        for loss_cls_i, loss_pts_i in zip(losses_cls[:-1], losses_pts[:-1]):
            ...
            loss_dict[f'd{num_dec_layer}.loss_boundary'] = losses_boundary[num_dec_layer]
            num_dec_layer += 1
        return loss_dict
    ...
    def get_sparse_voxels(self, voxel_semantics, mask_camera, mask_boundary):
        ...
        gt_points, gt_masks, gt_labels, gt_boundary_labels = [], [], [], []
        for i in range(B):
            ...
            if self.transformer.with_boundary_head:
                gt_boundary_labels.append(mask_boundary[i][mask]) # boundary mask

        return gt_points, gt_masks, gt_labels, gt_boundary_labels
