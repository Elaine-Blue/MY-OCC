...
class SSDOCCHeadV2(BaseModule):
    ...
    def loss(self, voxel_semantics, mask_camera, preds_dicts):
        ...
        losses_cls, losses_pts = multi_apply(
            ...
            all_gt_masks_list, all_gt_labels_list)
        ...
        if init_points is not None:
            ...
            _, init_loss_pts = self.loss_single(
                pseudo_scores, init_points, gt_points_list, gt_masks_list, gt_labels_list)
    ...
    def get_occ(self, pred_dicts, img_metas, rescale=False):
        ...
        for i in range(batch_size):
            refine_pts_i, cls_scores_i = refine_pts[i], cls_scores[i]
            ...
            mask_score = (cls_scores_i > score_thr).any(dim=-1)
            mask = mask_dist & mask_score
            refine_pts_i = refine_pts_i[mask]
            cls_scores_i = cls_scores_i[mask]
            pts = torch.cat([refine_pts_i, cls_scores_i], dim=-1)
