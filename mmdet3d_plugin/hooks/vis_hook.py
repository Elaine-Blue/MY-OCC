...
def create_color_legend(SL=20, LW=20, TL=180, font_size=20):
...
def visualize_occ(x, y, z, labels, palette, voxel_size, pc_range, init_points=None):
    if palette.max() > 1.0:
    ...
    if init_points is not None:
        x_valid = np.concatenate([x_valid, init_points[..., 0]])
        y_valid = np.concatenate([y_valid, init_points[..., 1]])
        z_valid = np.concatenate([z_valid, init_points[..., 2]])
        
        init_colors = [(255, 0, 255) for _ in range(len(init_points))]
        colors.extend(init_colors)
...
def draw_matched_results(matched_results, seq_idx, save_path, sample_interval=500):
    gt_paired_pts = matched_results['gt_paired_pts'][seq_idx][::sample_interval].detach().cpu().numpy()
    pred_paired_pts = matched_results['pred_paired_pts'][seq_idx][::sample_interval].detach().cpu().numpy()
    gt_paired_labels = matched_results['gt_paired_labels'][seq_idx][::sample_interval].detach().cpu().numpy()
    pred_paired_labels = matched_results['pred_paired_labels'][seq_idx][::sample_interval].detach().cpu().numpy()
    ...
    legend = create_color_legend(SL=60, LW=60, TL=540, font_size=60)
    lw, lh = legend.shape[1], legend.shape[0]
    save_fig[10:10+lh, 10:10+lw] = legend
    cv2.imwrite(save_path, save_fig)

def visualize_results(pc_range, voxel_size, results, matched_results, data_batch, save_dir, epoch, mode='cube'):
    ...
    for i in range(B):
        ...
        label, pos = results['sem_pred'][i].reshape(-1, 1), results['occ_loc'][i].reshape(-1, 3)
        ...
        if matched_results is not None:
            save_img_path = osp.join(save_dir, f'epoch_{epoch:0>3}_{i:0>6}_{scene_name}_{sample_token}_matched_pair.jpg')
